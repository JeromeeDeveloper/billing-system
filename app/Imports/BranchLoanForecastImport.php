...
class BranchLoanForecastImport implements ToCollection, WithHeadingRow
{
    ...
    public function __construct(string $billingPeriod, int $branchId)
    {
    ...
    public function collection(Collection $rows)
    {
        ...
        if (!$branch) {
            throw new \Exception("Branch with ID {$this->branchId} not found.");
        }
        ...
        foreach ($rows as $row) {
            ...
            // Check if this row belongs to the specified branch
            $branchCode = $row['branch_code'];
            Log::info("Processing row - CSV branch_code: '{$branchCode}', DB branch code: '{$branch->code}', Match: " . ($branch->code === $branchCode ? 'YES' : 'NO'));

            // TEMPORARILY: Process all rows to see what's happening
            // if ($branch->code !== $branchCode) {
            //     $this->stats['wrong_branch']++;
            ...
            $cid = $row['cid'];
            Log::info("Processing member CID: {$cid}, Name: {$row['name']}, Branch: {$branchCode}");
            ...
            if (!$member) {
                ...
                Log::info("BranchLoanForecast Import - Skipped CID {$cid}: Member not found or not tagged as PGB or New");
                $this->stats['not_found']++;
                continue;
            }
            ...
            if ($productCodePart) {
                ...
                if ($loanProducts->count() > 0) {
                    ...
                    Log::info("BranchLoanForecast Import - No loan product found for product code: {$productCodePart}, skipping loan forecast creation/update for CID: {$cid}");
                    $this->stats['skipped_no_product']++;
                    continue; // Skip this row entirely
                }
            } else {
                // If no product code, skip processing
                Log::info("BranchLoanForecast Import - No product code found in loan account: {$row['loan_account_no']}, skipping for CID: {$cid}");
        ...
        Log::info("BranchLoanForecast Import completed for Branch {$branch->name} - Processed: {$this->stats['processed']}, Skipped: {$this->stats['skipped']}, Not Found: {$this->stats['not_found']}, Wrong Branch: {$this->stats['wrong_branch']}");
        Log::info("Branch details - ID: {$branch->id}, Name: {$branch->name}, Code: {$branch->code}");
    }
