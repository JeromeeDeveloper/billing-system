...
<body>
    ...
    <script>
        ...
        function refreshStats() {
            fetch('/Branch/file-retention/stats')
        ...
        function viewFiles(documentType) {
            ...
            fetch('/Branch/file-retention/files?document_type=' + encodeURIComponent(documentType))
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
        ...
        function populateBillingPeriodFilter(files) {
            const billingPeriods = Array.from(new Set(files.map(f => f.billing_period)));
            billingPeriods.sort();
            let options = '<option value="">All Billing Periods</option>';
            billingPeriods.forEach(period => {
                options += `<option value="${period}">${period || 'N/A'}</option>`;
            });
            $('#billingPeriodFilter').html(options);
        }

        function filterFiles() {
            const selectedPeriod = $('#billingPeriodFilter').val();
            const searchTerm = $('#searchFilter').val().toLowerCase();

            const filteredFiles = allFiles.filter(file => {
                const matchesPeriod = selectedPeriod === '' || file.billing_period === selectedPeriod;
                const matchesSearch = file.filename.toLowerCase().includes(searchTerm);
                return matchesPeriod && matchesSearch;
            });
            displayFiles(filteredFiles, $('#modalDocumentType').text());
        }
            ...
            Swal.fire({
                ...
                html: `
                    <div class="text-center">
                        <p><strong>Filename:</strong> ${allFiles.find(f => f.id === fileId).filename}</p>
                        <p><strong>Size:</strong> ${(allFiles.find(f => f.id === fileId).size_bytes / 1024 / 1024).toFixed(2)} MB</p>
                        <p><strong>Upload Date:</strong> ${allFiles.find(f => f.id === fileId).upload_date}</p>
                        <p><strong>Billing Period:</strong> ${allFiles.find(f => f.id === fileId).billing_period || 'N/A'}</p>
                    </div>
                `,
                icon: 'info',
                ...
                if (result.isConfirmed) {
                    // No download functionality in this modal, just close
                } else if (result.isDismissed) {
                    const file = allFiles.find(f => f.id === fileId);
                    if (file && file.size_bytes < 1024 * 1024) { // Small files can be downloaded directly
                        const link = document.createElement('a');
                        link.href = `/Branch/file-retention/download/${file.id}`;
                        link.download = file.filename;
                        document.body.appendChild(link);
                        link.click();
                        document.body.removeChild(link);
                    } else {
                        Swal.fire('Info', 'This file is too large to download directly in the browser. Please contact support.', 'info');
                    }
                }
            });
        }
        ...
        function deleteFile(fileId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = `/Branch/file-retention/delete/${fileId}`;
        ...
        function previewCleanup(documentType = null) {
            const url = '/Branch/file-retention/preview';
        ...
        function performCleanup(documentType) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = '/Branch/file-retention/cleanup-type';
        ...
        function performCleanupAll() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = '/Branch/file-retention/cleanup-all';
        ...
        function createBackup() {
            ...
            fetch('/Branch/file-retention/backup', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
