...
document.addEventListener('DOMContentLoaded', function() {
	...
	console.log('Remittance Data Count:', remittanceData.length);
	console.log('Remittance Tags:', remittanceTags);
	if (remittanceData.length > 0) {
		console.log('First data item:', remittanceData[0]);
	}
	...
	function generateHeaders(section) {
		...
		switch(section) {
			case 'summary':
				headers = ['CID', 'Member Name', 'Type', 'Remitted Loans', 'Remitted Savings', 'Remitted Shares', 'Total Remitted'];
				break;
			case 'loans':
	...
	function generateTableData(section) {
		...
		Object.keys(groupedData).forEach(cid => {
			...
			switch(section) {
				case 'summary':
					const totalLoans = memberReports.filter(r => r.remittance_type === 'loans_savings').reduce((sum, r) => sum + parseFloat(r.remitted_loans || 0), 0);
					const totalSavings = memberReports.filter(r => r.remittance_type === 'loans_savings').reduce((sum, r) => sum + parseFloat(r.remitted_savings || 0), 0);
				...
				case 'loans':
					const loansReports = memberReports.filter(r => r.remittance_type === 'loans_savings');
					const totalLoansAmount = loansReports.reduce((sum, r) => sum + parseFloat(r.remitted_loans || 0), 0);
					const billedAmount = loansReports.reduce((sum, r) => sum + parseFloat(r.billed_amount || 0), 0);
					...
					if (totalLoansAmount > 0) {
						...
						remittanceTags.forEach(tag => {
							...
							const amount = report ? parseFloat(report.remitted_loans || 0) : 0;
							row.push(amount);
							totalPaid += amount;
						});
				...
				case 'savings':
					const savingsReports = memberReports.filter(r => r.remittance_type === 'loans_savings');
					const totalSavingsAmount = savingsReports.reduce((sum, r) => sum + parseFloat(r.remitted_savings || 0), 0);
					...
					if (totalSavingsAmount > 0) {
						...
						remittanceTags.forEach(tag => {
							const report = savingsReports.find(r => r.remittance_tag == tag);
							const amount = report ? parseFloat(report.remitted_savings || 0) : 0;
							row.push(amount);
							totalPaid += amount;
						});
				...
				case 'shares':
					...
					const totalSharesAmount = sharesReports.reduce((sum, r) => sum + parseFloat(r.remitted_shares || 0), 0);
					...
					if (totalSharesAmount > 0) {
						...
						remittanceTags.forEach(tag => {
							...
							const amount = report ? parseFloat(report.remitted_shares || 0) : 0;
							row.push(amount);
							totalPaid += amount;
						});
		...
		if (rows.length === 0) {
			...
			tableBody.innerHTML = `<tr><td colspan="${colCount}" class="text-center text-muted">No data available for the selected section and filters.</td></tr>`;
		} else {
	...
	if (remittanceData.length === 0) {
		...
		tableBody.innerHTML = '<tr><td colspan="7" class="text-center text-muted">No remittance data available. Please upload remittance files first.</td></tr>';
	} else {
		updateTable();
	}
});
